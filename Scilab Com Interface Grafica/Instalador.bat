...
set RotinaPrincipal=A0_MAIN.sce
set ArquivoExecutor=Analise_Dinamica_Estruturas_Gui.bat
set MascaraSoftware=C:\WScilex.exe
set TerminacaoSoftware=scilab-6.0.2\bin\WScilex.exe
...
if "%linha:~-29%=="%TerminacaoSoftware%" (set ExecutavelSoftware=%linha%)
...
:end
::Customizado para o Scilab 6.0.2
...
echo[
echo Necessario ter instalado o Scilab 6.0.2
echo[
echo[
goto :pausafinal
)
...
:: Obs: este executa o programa com a janela normal. O de baixo executaria com a janela minimizada
:: Obs: abrir o scilab completo e centralizar caixas de mensagens
echo @call start "" %ExecutavelSoftware% -e "chdir(\"Engine\");exec(\"%RotinaPrincipal%\",-1);" > %ArquivoExecutor%
...
REM echo @call %ExecutavelSoftware% -e "atomsInstall(\"IPCV\");quit;" > ImageToolBox.bat
...
:: --------------------------------------------------------------------------------------------------
:: Escreve no arquivo "Configuracao.bat" a linha de comando abaixo, usando a variavel do Executavel
echo @call %ExecutavelSoftware% -e "mputl(SCIHOME,\"Endereco.txt\");quit;" > Configuracao.bat

:: Excuta o arquivo "Configuracao.bat" que tem a linha de comando acima
echo --------------------------------
echo Copiando arquivos de configuracao  . . .
echo[
@call Configuracao.bat

:: Exclui o arquivo "Configuracao.bat"
del Configuracao.bat

for /F "tokens=*" %%i in (Endereco.txt) do call :Novasubrotina "%%i"
goto NewEnd
:Novasubrotina
set linha=%1
copy /Y "Engine\configuration.xml" %linha%
copy /Y "Engine\windowsConfiguration.xml" %linha%
copy /Y "Engine\XConfiguration.xml" %linha%
goto :eof
:NewEnd
:: Exclui o arquivo "Endereco.txt"
del Endereco.txt
:: --------------------------------------------------------------------------------------------------
:: Informa mensagens no prompt
echo --------------------------------
